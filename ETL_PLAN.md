...
- [ ] Implement Spark readers for each source format
- [ ] Handle null representations per source
- [ ] Write to Parquet with `ingest_date` partition

### Phase 2: Silver Layer (Cleaning)
- [ ] Normalize IDs across all sources
- [ ] Parse and validate box office values
- [ ] Build entity resolution spine from links.csv
- [ ] Standardize genre mappings

### Phase 3: Gold Layer (Star Schema)
- [ ] Build dimension tables with proper sourcing priority
- [ ] Aggregate MovieLens ratings for fact table
- [ ] Calculate derived metrics (divisive_score)
- [ ] Build bridge tables with proper FK relationships

### Phase 4: PostgreSQL Load
- [ ] Create staging tables in PostgreSQL
- [ ] Implement JDBC batch loading
- [ ] Add upsert/truncate-insert logic
...
```json
{"rt_id":"zootopia_2","title":"Zootopia 2","tomatometer_score":91,"audience_score":null,"mpaa_rating":"PG","box_office":null,"director":"Jared Bush, Byron Howard"}
```

### TMDB NDJSON
```json
{"id":11224,"title":"Cinderella","genre_ids":[10751,14,16,10749],"poster_path":"/4nssBcQUBadCTBjrAkX46mVEKts.jpg","vote_average":7.045}
```
